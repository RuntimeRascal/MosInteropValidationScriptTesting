            ...
            Else ' Start Validation-Code
                ...
                Try
                    For Each shp As Microsoft.Office.Interop.PowerPoint.Shape In Me._myOfficeApp.ActivePresentation.Slides(4).Shapes
                        'Insert Callout
                        Try
                            If InStr(shp.Name, programingWords(5)) Or InStr(shp.Name, "Callout") Then 'Hardcoded "Callout" because some languages don't translate that -Daniel 12/23/14
                                points(0) = True
                                'Callout Sytle and size and Callout Text								
                                If shp.Height > 111 And shp.Height < 114 Then
                                    If shp.Width > 217 And shp.Width < 220 Then
                                        If shp.ShapeStyle = MsoShapeStyleIndex.msoShapeStylePreset40 Then
                                            If InStr(shp.TextFrame.TextRange.Text.ToUpper, programingWords(6).ToUpper) Then 'Voor groen gaan helpt iedereen
                                                points(1) = True
                                            End If
                                        End If
                                    End If
                        ...
                        'Insert Picture
                        Try
                            If shp.PictureFormat.ColorType = MsoPictureColorType.msoPictureAutomatic Then
                                points(2) = True
                                'Picture Reflection
                                If shp.Reflection.Size = 30 Then
                                    If shp.Reflection.Type = MsoReflectionType.msoReflectionTypeMixed Then
                                        points(3) = True
                                    End If
                                End If
                                If shp.Title = programingWords(7) Then
                                    points(4) = True
                                End If
                            End If
